...
/// <summary>
/// Defence-in-depth backstop keeping the service surfaces LAN/WireGuard-only.
///
/// <para>
/// The PRIMARY control is the Cloudflare Tunnel ingress simply not routing <c>/mcp</c>, <c>/internal</c>,
/// or <c>/dav-backend</c> to the container (host config, outside this repo). This middleware is the
/// belt-and-suspenders that survives an ingress mistake: Cloudflare's edge stamps <c>CF-Ray</c>/
/// <c>CF-Connecting-IP</c> on everything that arrives through the tunnel, and a direct LAN/WireGuard
/// request to the container port never carries them — so a tunnelled request to these prefixes is
/// answered with 404 (indistinguishable from "no such route") before it reaches a handler.
/// </para>
///
